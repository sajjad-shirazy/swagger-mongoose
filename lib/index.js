...
module.exports.compile = function(spec, _extraDefinitions) {
    ...
    _.forEach(definitions, function(definition, key) {
        ...
        if (object && !excludedSchema) {
            ...
            let hasRef = false;
            _.forEach(schema.tree, column => column && column.ref ? hasRef = true : null);
            if (hasRef) {
                schema.plugin(mongooseRefValidator, {
                    // onDeleteRestrict  
                    // https://github.com/getlackey/mongoose-ref-validator          
